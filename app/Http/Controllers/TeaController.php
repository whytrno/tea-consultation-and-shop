...
class TeaController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($data as $d) {
            $d->image = url('images/tea') . '/' . $d->image;
        }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'name' => 'required|max:255|unique:teas,name',
        ...
        $image->move(public_path('images/tea'), $image_name);
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            'name' => 'nullable|max:255|unique:teas,name,' . $id,
        ...
        if ($request->hasFile('image')) {
            $image_path = public_path('images/tea/') . $data->image;
